"""Generate correlation coefficient plots for merged reflection data."""
...
def calc_cc(df, *cols):
    s_bin = pd.cut(df.s, np.linspace(0.03, 0.8, 78))
...
def calc_cc2(df1, df2, col):
    ...
    df12.head()
    s_bin = pd.cut(df12.s_1, np.linspace(0.03, 0.8, 78))
    df_tmp = (
        df12.groupby(s_bin)[[col + "_1", col + "_2"]]
        .corr(method="pearson")
        .unstack()
    )
...
#Check to see if you might want to specify the crystals you want to compare (not just crystals 1 and 2; also, might want to compare multiple crystals)
@dataclass
class Parameters:
    """Options for generating the correlation coefficient figure."""

    # Currently no command-line options; this dataclass exists for consistency.
    pass


def run_mapstats2(params: Parameters):
...
run = with_parsing(parse_arguments)(with_logging()(run_mapstats2))
